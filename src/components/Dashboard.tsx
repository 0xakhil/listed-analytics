...
function ChartCard({ title, subtitle, rows, dataKey, color, range, bars = false }: { title: string; subtitle: string; rows: ListedDashboardPayload["days"]; dataKey: ChartKey; color: string; range: DashboardRange; bars?: boolean }) {
  const chartLabel = dataKey.includes("cumulative") ? `Cumulative ${dataKey.includes("Fees") ? "fees" : "volume"}` : dataKey === "fees" ? "Fees" : "Volume";
  return <section className="rounded-[22px] border border-line bg-surface p-5 sm:p-6">
    ...
    <div className="h-64"><ResponsiveContainer width="100%" height="100%">{bars ? <BarChart data={rows} margin={{ top: 8, right: 4, left: -16, bottom: 0 }}>
      <CartesianGrid stroke="#2a2a2e" strokeDasharray="3 5" vertical={false} /><XAxis dataKey="date" tick={{ fill: "#8d8a84", fontSize: 11 }} tickLine={false} axisLine={false} minTickGap={22} tickFormatter={(value) => String(value).slice(5)} />
      ...
      <Tooltip cursor={{ fill: "#ffffff08" }} contentStyle={{ background: "#171719", border: "1px solid #343238", borderRadius: 10, fontSize: 12 }} formatter={(value) => [usd(Number(value ?? 0), 2), chartLabel]} /><Bar dataKey={dataKey} fill={color} radius={[5, 5, 0, 0]} />
    </BarChart> : <AreaChart data={rows} margin={{ top: 8, right: 4, left: -16, bottom: 0 }}>
      ...
      <CartesianGrid stroke="#2a2a2e" strokeDasharray="3 5" vertical={false} /><XAxis dataKey="date" tick={{ fill: "#8d8a84", fontSize: 11 }} tickLine={false} axisLine={false} minTickGap={22} tickFormatter={(value) => String(value).slice(5)} />
      <YAxis tick={{ fill: "#8d8a84", fontSize: 11 }} tickLine={false} axisLine={false} tickFormatter={(value) => `$${Number(value).toLocaleString("en-US", { notation: "compact" })}`} width={58} />
      <Tooltip cursor={{ stroke: "#525057", strokeDasharray: "3 3" }} contentStyle={{ background: "#171719", border: "1px solid #343238", borderRadius: 10, fontSize: 12 }} formatter={(value) => [usd(Number(value ?? 0), 2), chartLabel]} />
...
export function Dashboard() {
  const [data, setData] = useState<ListedDashboardPayload | null>(null); const [error, setError] = useState(false); const [range, setRange] = useState<DashboardRange>("30d");
  useEffect(() => {
    let active = true;
  ...
  const rows = useMemo(() => !data ? [] : range === "all" || range === "30d" ? data.days : data.days.slice(range === "7d" ? -7 : -1), [data, range]);
  ...
  const activity = data.windows[range]; const updated = new Date(data.updatedAt ?? data.generatedAt).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" });
  return <main className="mx-auto max-w-6xl px-5 py-8 sm:px-8 sm:py-10">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Metric label="Volume" value={usd(activity.volume, 2)} note="Confirmed swap notional" /><Metric label="Cumulative volume" value={usd(data.windows.all.volume, 2)} note="All confirmed activity" /><Metric label="Platform users" value={whole(activity.users)} note="Wallets with a confirmed trade" /><Metric label="One-trade wallets" value={whole(activity.oneTradeUsers)} note="Exactly one trade in this period" /><Metric label="Trades" value={whole(activity.swaps)} note="Confirmed swaps" /></section>
    <section className="mt-3 grid gap-3 lg:grid-cols-2"><ChartCard title="Daily trading volume" subtitle="Confirmed swap volume" rows={rows} dataKey="volume" color="#37e39a" range={range} bars /><ChartCard title="Cumulative volume" subtitle="All-time growth" rows={rows} dataKey="cumulativeVolume" color="#f5a524" range={range} /></section>
...
function Metric({ label, value, note }: { label: string; value: string; note: string }) { return <article className="rounded-[22px] border border-line bg-surface p-5 sm:p-6"><p className="text-xs font-medium uppercase tracking-[0.14em] text-mute">{label}</p><p className="mt-3 text-3xl font-semibold tracking-[-0.045em] text-ivory">{value}</p><p className="mt-2 text-xs text-mute">{note}</p></article>; }
